...
$(document).on('click', '#showGridBtn', function () {
    Class.ClassAucSubmit(this.value);
});
window.Class = {
    ...
    PledgeDetails: async function () {
        debugger;
        document.getElementById("div_aucGrid").style.display = "none";
        document.getElementById("div_auc").style.display = "none";
        ...
        if (PledgeValue === "-1") {
            ...
            try {
                ...
                else if (responseData.err_code === "2") {
                    document.getElementById('pledg_drp').value = "-1";
                    await showAlert("Alert!", "This action will be available after the auction reverse transfer of the pledge is completed..", "warning");
                    return;
                }
                else {
    ...
    ClassAucSubmit: async function () {
        ...
        try {
            ...
            Res = decryptAES(Res);
            const responseData = JSON.parse(Res);
            if (responseData.err_code === "2") {

                const parsedOutdata = JSON.parse(responseData.outdata);

                const data = parsedOutdata.Table[0];
                document.getElementById("div_auc").style.display = "block";
                document.getElementById("div_aucGrid").style.display = "none";

                document.getElementById("Auc_sts").value = Object.values(data)[0];
            }
            else if (responseData.err_code === "3") {
                const outdata = JSON.parse(responseData.outdata);
               document.getElementById("div_aucGrid").style.display = "block";
                document.getElementById("div_auc").style.display = "none";
