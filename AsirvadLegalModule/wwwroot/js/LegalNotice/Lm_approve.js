...
$(document).on("input", "#lmRemarks", function () {
...
var _approve = {
    ...
    noticeTypeDropdown: async function () {
        ...
        try {
            const requestData = {
                "employeeId": sessionStorage.getItem("EmployeeId"),
                "token": sessionStorage.getItem("Token"),
                "indata": encryptAES($("#branch").val() + "~" + "1"),
                "BranchId": sessionStorage.getItem("BranchId"),
                "enindata": sessionStorage.getItem("BranchId"),
                "flag": encryptAES("8")
            };
    ...
    ah_update: async function () {
        try {
            ...
            if (document.getElementById('lmRemarks').value == "") {
                await showAlert("Alert!", "Please Enter Remarks..!!");
                return;
            }
            var data = {
                "indata": encryptAES($("#noticeType").val() + "~" + $("#lmRemarks").val()),
                "employeeId": sessionStorage.getItem("EmployeeId"),
                "token": sessionStorage.getItem("Token"),
                "BranchId": sessionStorage.getItem("BranchId"),
                "flag": encryptAES("9"),
            ...
            if (data2.status == "True") {
                await showSuccessAlert("success!", "Successfully Updated", "/LegalNotice/LegalLm_recommend");
            }
            else {
                await showAlert("Alert!", "Error occured..Please try again..", "warning");
            }
    ...
    lm_reject: async function () {
        try {
            ...
            var data = {
                "indata": encryptAES($("#noticeType").val() + "~" + $("#lmRemarks").val()+"~"+"3"),
                "employeeId": sessionStorage.getItem("EmployeeId"),
                "token": sessionStorage.getItem("Token"),
                "BranchId": sessionStorage.getItem("BranchId"),
                "flag": encryptAES("15"),
            ...
            let data2 = JSON.parse(Res);
            if (data2.status == "True") {
                await showSuccessAlert("success!", "Successfully Returned", "/LegalNotice/LegalLm_recommend");
            }
            else {
                await showAlert("Alert!", "Error occured..Please try again..", "warning");
            }
