...
{
    public class ChequeVerificationController : Controller
    {
        PrivateAPIManager privateAPI;
        public ChequeVerificationController(PrivateAPIManager privateAPI)
        {
            this.privateAPI = privateAPI;
        }
        public IActionResult ChequeVerify()
        {
            return View("~/Views/ChequeCollection/ChequeVerification.cshtml");
        }



        [HttpPost("IrrSelect")]
        public async Task<IActionResult> IrrSelect([FromBody] ChequeUpdationRequest request)
        {
            try
            {
